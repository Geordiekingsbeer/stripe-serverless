// File: /api/webhook.js
// FINAL DEFINITIVE VERSION: Simplified fulfillment payload to ensure database INSERT success.
...
export default async function (req, res) {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        // Tracking Metadata (CRITICAL FOR BOTH INSERTS)
        const tenantId = session.metadata.tenant_id;
        const bookingRef = session.metadata.booking_ref;

        // Check if metadata is missing (should be caught by create-checkout, but safe check)
        ...
        if (existingTrackingData && existingTrackingData.payment_successful === true) {
            ...
            await logConversionStatusUpdate(tenantId, bookingRef); // Ensure tracking update runs
            return res.status(200).json({ received: true, status: 'Already Processed' });
        }

        // --- PREPARE DATA FOR premium_slots INSERTION ---
        ...
        const bookingsToInsert = tableIdsArray.map(tableId => ({
            ...
            // NOTE: host_notes is omitted to prevent any column constraint errors.
        }));
        ...
        if (insertError) {
            ...
            // Returning a 500 signals Stripe to retry, which is correct since fulfillment failed.
            return res.status(500).json({ received: false, status: 'Fulfillment Insert Failed' });
        }
        
        // 4b. LOG PAYMENT SUCCESS STATUS (Update the single tracking row)
